...
이미 설치되어 있다면 로컬 변경이 없는지 먼저 확인한 후 업데이트합니다.
...
이 저장소를 확인한 AI는 다음 절차를 따릅니다.

1. 저장소 루트의 `SKILL.md`와 `agents/openai.yaml`을 확인합니다.
2. `CODEX_HOME`이 있으면 `$CODEX_HOME/skills/gpt56-model-router`, 없으면 `~/.codex/skills/gpt56-model-router`를 설치 위치로 선택합니다.
3. 대상이 없으면 저장소를 해당 경로에 복제합니다.
4. 대상이 이미 있으면 로컬 변경을 먼저 확인하고, 사용자 파일을 덮어쓰지 않습니다. 안전할 때만 `git pull --ff-only`로 업데이트합니다.
5. `SKILL.md`, `agents/`, `references/`, `assets/`가 존재하는지 확인합니다.
6. 가능하면 아래 검증 명령을 실행합니다.
7. 설치 완료 후 새 세션에서 스킬을 명시적으로 호출하도록 안내합니다.
